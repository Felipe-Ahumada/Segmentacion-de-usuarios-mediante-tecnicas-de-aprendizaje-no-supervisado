...
if audiencia == "Supervisado":
    st.title("Modelos Predictivos — Plataforma de Streaming")
    st.caption("Clasificación supervisada y regresión sobre los segmentos identificados por KMeans.")
else:
...
# Vista supervisado
    ...
    tab_ejec, tab_tec, tab_oper = st.tabs(["Ejecutiva", "Técnica", "Operativa"])

    # ── Tab Ejecutiva ────────────────────────────────────────────────
    with tab_ejec:
        st.caption("Resumen de alto nivel de los modelos predictivos.")
        ...
        st.markdown(
            f"El clasificador **{mejor_cls}** permite asignar automáticamente a nuevos usuarios "
            f"al segmento correcto con un F1 macro de **{mejor_cls_res['f1_macro']:.1%}**."
        )
        ...
        st.markdown(
            f"El regresor **{mejor_reg}** estima el gasto mensual con un R² de "
            f"**{mejor_reg_res['r2']:.3f}**, útil para anticipar el valor económico de cada cliente."
        )

    # ── Tab Técnica ──────────────────────────────────────────────────
    with tab_tec:
        st.caption("Detalle de modelos, métricas, gráficos y comparación.")

        st.header("Clasificación de segmentos")
        st.markdown(
            "Los segmentos identificados por KMeans se usan como **variable objetivo** para entrenar "
            "clasificadores supervisados con `Pipeline(Winsorizer + StandardScaler + CorrelationFilter + modelo)` "
            "y optimización en dos etapas (RandomizedSearchCV → GridSearchCV, validación cruzada de 5 folds)."
        )
        ...
        with st.expander("Ver hiperparámetros óptimos por modelo (clasificación)"):
            for nombre, res in cls_data["resultados"].items():
        ...
        st.markdown(
            ...
            "el mismo pipeline de preprocesamiento con optimización en dos etapas."
        )
        ...
        with st.expander("Ver hiperparámetros óptimos por modelo (regresión)"):
            for nombre, res in reg_data["resultados"].items():
                if res["best_params"]:
                    params_str = ", ".join(f"{k}={v}" for k, v in res["best_params"].items())
                    st.markdown(f"**{nombre}**: {params_str}")
    ...
    # ── Tab Operativa ────────────────────────────────────────────────
    with tab_oper:
        st.caption("Detalle operativo de los modelos supervisados: resumen, variables y simulador.")

        st.header("Modelos en producción")
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Mejor clasificador")
            st.metric("Modelo", mejor_cls)
            st.metric("F1 macro", f"{mejor_cls_res['f1_macro']:.1%}")
            st.metric("Accuracy", f"{mejor_cls_res['accuracy']:.1%}")
        with col2:
            st.subheader("Mejor regresor")
            st.metric("Modelo", mejor_reg)
            st.metric("R²", f"{mejor_reg_res['r2']:.3f}")
            st.metric("MAE", f"${mejor_reg_res['mae']:,.0f}")

        st.divider()

        st.header("Resultados por modelo")
        st.subheader("Clasificadores")
        filas_cls_op = []
        for nombre, res in cls_data["resultados"].items():
            filas_cls_op.append({
                "Modelo": nombre,
        ...
        st.dataframe(pd.DataFrame(filas_cls_op), hide_index=True, use_container_width=True)

        st.subheader("Regresores")
        filas_reg_op = []
        for nombre, res in reg_data["resultados"].items():
            filas_reg_op.append({
                "Modelo": nombre,
        ...
        st.dataframe(pd.DataFrame(filas_reg_op), hide_index=True, use_container_width=True)

        st.divider()

        st.header("Variables utilizadas")
        features_cls = list(cls_data["resultados"][mejor_cls].get("best_params", {}).keys())
        st.subheader("Features de clasificación")
        st.markdown(", ".join(f"`{v}`" for v in scaler_cols) if (scaler_cols := list(ETIQUETAS_VAR.keys())) else "No disponible")
        st.subheader("Features de regresión")
        features_reg_list = reg_data.get("features", [])
        if features_reg_list:
            st.markdown(", ".join(f"`{ETIQUETAS_VAR.get(v, v)}`" for v in features_reg_list))
        else:
            st.markdown("Todas las variables excepto el target (`gasto_mensual`)")
